...
for event in dataTree:
    ...
    for d in range(1, HBdepths):
        HB9 += HcalEnergyAround(event.HBE, ieta, iphi, depth=d, radius=radius,
                                calib=trivialConst)

    EB81 = EcalEnergyAround(event.EBE, ecalXtalieta, ecalXtaliphi, radius=4)\
           *trivialConst[0]

    if ((HB9+EB81) < beamE - 3.*resolution):
        # print 'HB9:',HB9
        continue
    ...
    ## if not ('HB9' in calibData.keys()):
    ##     calibData['HB9'] = []
    ## calibData['HB9'].append(HB9)
    ...
    for e in range(ieta-radius, ieta+radius+1):
        for p in range(iphi-radius, iphi+radius+1):
            ...
            if not isInstrumented('HO', e, p, 4):
                tmpIndex = -1
            if (tmpIndex > 0):
                Ename = 'ho_{0}_{1}'.format(e,p)
            if not (Ename in calibData.keys()):
                calibData[Ename] = []
            calibData[Ename].append(event.HOE[tmpIndex]*\
...
for par in range(1, minner.GetNumberTotalParameters()):
    ...
    if (parName[:2] == 'ho'):
        ...
        if calIndex >= 0:
            ...
            print 'initial value: {0:0.3f}'.format(trivialConstHO[calIndex]),
            if (parSignif > 0.5):
...
for event in dataTree:
    ...
    sumBefore += HcalEnergyAround(event.HOE, ieta, iphi, radius=radius,
                                  calib=trivialConstHO)
    sumAfter += HcalEnergyAround(event.HOE, ieta, iphi, radius=radius,
                                 calib=calibConstHO)
...
storageFile = 'ho_calib_{0}_{1}.pkl'.format(ieta, iphi)
storeCalibration(calibConstHO, storageFile)
print 'file {0} written'.format(storageFile)
