...
SiPMModel::SiPMModel() : _NP(0), _tau(0), _crossTalk(0), _tempDep(0),
			 theLastHitTime(-1)
{ 
}

SiPMModel::SiPMModel(unsigned int NP, double tau) :
  _NP(NP), _SiPM(NP), _tau(tau), _crossTalk(0), _tempDep(0),
  theLastHitTime(-1)
{
  resetSiPM();
}
...
SiPMModel::SiPMModel(SiPMModel const& other) :
  _NP(other._NP), _SiPM(other._SiPM), _tau(other._tau),
  _crossTalk(other._crossTalk), _tempDep(other._tempDep), 
  theLastHitTime(other.theLastHitTime)
{
}

  double SiPMModel::hitPixels(unsigned int pes, double tempDiff,
			      double photonTime) {
  ...
  for (unsigned int pe = 0; pe < pes; ++pe) {
    ...
    hit = (_SiPM[pixel] < 0.) ? 1.0 : cellCharge(photonTime - _SiPM[pixel]);
    sum += hit*(1 + (tempDiff*_tempDep));
    _SiPM[pixel] = photonTime;
  }
...
double SiPMModel::cellCharge(double deltaTime) const {
  if (deltaTime <= 0.) return 0.;
  if (deltaTime > 10.*_tau) return 1.;
  double result(1. - std::exp(-deltaTime/_tau));
