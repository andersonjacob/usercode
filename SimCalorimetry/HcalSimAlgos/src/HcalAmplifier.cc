...
void HcalAmplifier::amplify(CaloSamples & frame) const {
  const CaloSimParameters& parameters = 
    theParameterMap->simParameters(frame.id());
  ...
  for(int tbin = 0; tbin < frame.size(); ++tbin) {
    ...
    if(addNoise_) {
      pedestal += noise [tbin];
      if ( (tmpId.subdet() == HcalBarrel) ||
	   (tmpId.subdet() == HcalEndcap) ||
	   (tmpId.subdet() == HcalOuter) ) {
	const HcalSimParameters& pars = 
	  dynamic_cast<const HcalSimParameters &>(parameters);
	// std::cout << tmpId
	// 	  << " extraNoise: " << pars.extraNoise()
	// 	  << " extraNoiseWidth: " << pars.extraNoiseWidth()
	// 	  << std::endl;
	if (pars.extraNoise() > 0) {
	  double extraNoise = theRandGaussQ->fire(pars.extraNoise(), 
						  pars.extraNoiseWidth());
	  if (extraNoise > 0) pedestal += extraNoise;
	}
      }
    }
    frame[tbin] *= fCperPE;
