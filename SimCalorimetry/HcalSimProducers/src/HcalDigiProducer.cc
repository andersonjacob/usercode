...
: theParameterMap(new HcalSimParameterMap(ps)),
  theHcalShape(new HcalSiPMShape()),
  ...
  theHBHEResponse(new HcalSiPMHitResponse(theParameterMap, theHcalIntegratedShape)),
  theHOResponse(new HcalSiPMHitResponse(theParameterMap, theHcalIntegratedShape)), 
  ...
  doZDC(true)
{
  ...
  //std::cout << "digi producer starting up." << std::endl;
...
void HcalDigiProducer::produce(edm::Event& e, const edm::EventSetup& eventSetup) {
  ...
  if (relabel_) {
    ...
    col=std::auto_ptr<MixCollection<PCaloHit> >(new MixCollection<PCaloHit>(theRelabeller->getCrossingFrame()));
  } else {
    col=std::auto_ptr<MixCollection<PCaloHit> >(new MixCollection<PCaloHit>(cf.product()));
  }  
...
void HcalDigiProducer::checkGeometry(const edm::EventSetup & eventSetup) {
  ...
  // std::cout << "valid HB ids: " << hbCells.size() 
  // 	    << " 4*72*16 = " << (4*72*16)
  // 	    << " 72*18 = " << (72*18)
  // 	    << std::endl;
